...
`WebTestClient` is a thin shell around xref:webflux-client:webflux-client.adoc#webflux-client[WebClient],
...
xref:unit-testing:mock-objects:mock-objects-web-reactive.adoc#mock-objects-web-reactive[mock request and response], or it can test any
web server over an HTTP connection.

TIP: Kotlin users: See xref:kotlin:kotlin-spring-projects-in-kotlin:testing.adoc#kotlin-webtestclient-issue[this section]
...
The preceding example loads the xref:webflux:webflux-config.adoc#webflux-config[WebFlux Java configuration]
...
xref:webflux:webflux-fn.adoc#webflux-fn[RouterFunction]:
...
processing chain. See xref:webflux:webflux-reactive-spring-web:webflux-web-handler-api.adoc#webflux-web-handler-api[WebHandler API] for
...
`WebTestClient` provides an API identical to xref:webflux-client:webflux-client.adoc#webflux-client[WebClient]
...
* `expectBody()`: Decode to `byte[]` for xref:integration-testing:webtestclient:webtestclient-tests.adoc#webtestclient-json[webtestclient-json] or an empty body.
...
xref:webflux-client:webflux-client-body.adoc#webflux-client-body[WebClient documentation] for examples on
