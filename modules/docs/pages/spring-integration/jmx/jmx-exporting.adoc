...
corresponding entry value. You can change this behavior, as described in xref:jmx:jmx-naming.adoc#jmx-naming[jmx-naming].
...
NOTE: `MBeanExporter` is a `Lifecycle` bean (see xref:beans:beans-factory-nature:beans-factory-lifecycle.adoc#beans-factory-lifecycle-processor[Startup and Shutdown Callbacks]). By default, MBeans are exported as late as possible during
...
The configuration shown in the xref:jmx:jmx-exporting.adoc#jmx-exporting[preceding section] assumes that the
...
xref:beans:beans-definition:beans-factory-class.adoc#beans-factory-class-static-factory-method[factory-method],
...
as detailed in xref:jmx:jmx-naming.adoc#jmx-naming[jmx-naming].
