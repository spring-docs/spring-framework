...
xref:transaction:transaction-declarative:transaction-declarative-first-example.adoc#transaction-declarative-first-example[advice]). The example
...
xref:aop:aop.adoc#aop[Aspect Oriented Programming with Spring]). In the preceding example,
