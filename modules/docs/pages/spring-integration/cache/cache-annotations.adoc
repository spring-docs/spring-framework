...
through its `key` attribute. You can use xref:expressions:expressions.adoc#expressions[SpEL] to pick the
arguments of interest (or their nested properties), perform operations, or even
invoke arbitrary methods without having to write any code or implement any interface.
This is the recommended approach over the
xref:cache:cache-annotations:cache-annotations-cacheable.adoc#cache-annotations-cacheable-default-key[default generator], since methods tend to be
...
do yourself a favor and read xref:expressions:expressions.adoc#expressions[Spring Expression Language]):
...
replacing xref:cache:cache-annotations:cache-annotations-cacheable.adoc#cache-annotations-cacheable-key[key generation]. The resolution is
...
Each `SpEL` expression evaluates against a dedicated xref:expressions:expressions-language-ref.adoc#expressions-language-ref[`context`].
...
xref:transaction:transaction-declarative:transaction-declarative-annotations.adoc#tx-annotation-driven-settings[`@Transactional`].
...
| The default mode (`proxy`) processes annotated beans to be proxied by using Spring's AOP
  ...
  xref:aop:aop-using-aspectj:aop-aj-ltw.adoc#aop-aj-ltw-spring[Spring configuration] for details on how to set up
  load-time weaving.)
...
| Defines the order of the cache advice that is applied to beans annotated with
  ...
  ordering AOP advice, see xref:aop:aop-ataspectj:aop-advice.adoc#aop-ataspectj-advice-ordering[Advice Ordering].)
...
See xref:mvc:mvc-servlet.adoc#mvc-servlet[the MVC section] for more information.
...
of the xref:beans:beans-classpath-scanning:beans-stereotype-annotations.adoc#beans-stereotype-annotations[stereotype] annotations, you can
use `@Cacheable`, `@CachePut`, `@CacheEvict`, and `@CacheConfig` as
xref:beans:beans-classpath-scanning:beans-meta-annotations.adoc#beans-meta-annotations[meta-annotations] (that is, annotations that
...
xref:cache:cache-annotations:cache-annotation-enable.adoc#cache-annotation-enable[earlier], annotation-driven behavior needs to be enabled.
