...
Spring provides xref:cache:cache-store-configuration.adoc#cache-store-configuration[a few implementations] of that abstraction:
...
compliant caches (such as Ehcache 3.x). See xref:cache:cache-plug.adoc#cache-plug[cache-plug] for more information on
