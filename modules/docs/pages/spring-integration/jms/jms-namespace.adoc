...
and `<jca-listener-container/>`. `<annotation-driven/>` enables the use of xref:jms:jms-annotated.adoc#jms-annotated[annotation-driven listener endpoints]. `<listener-container/>` and `<jca-listener-container/>`
...
in xref:jms:jms-receiving:jms-receiving-async-message-listener-adapter.adoc#jms-receiving-async-message-listener-adapter[jms-receiving-async-message-listener-adapter]. In addition to the attributes shown
