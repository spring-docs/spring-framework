...
that does not tie a user to an EJB container. (See xref:jms:jms-receiving:jms-receiving-async.adoc#jms-receiving-async[jms-receiving-async] for detailed
coverage of Spring's MDP support.) Since Spring Framework 4.1, endpoint methods can be
annotated with `@JmsListener` -- see xref:jms:jms-annotated.adoc#jms-annotated[jms-annotated] for more details.
...
* xref:jms:jms-using:jms-mdp.adoc#jms-mdp-simple[`SimpleMessageListenerContainer`]
* xref:jms:jms-using:jms-mdp.adoc#jms-mdp-default[`DefaultMessageListenerContainer`]
...
NOTE: Like its sibling (xref:jms:jms-using:jms-mdp.adoc#jms-mdp-simple[`SimpleMessageListenerContainer`]),
...
xref:transaction:transaction.adoc#transaction[Transaction Management section of the Data Access chapter].
