...
See xref:jms:jms-annotated:jms-annotated-support.adoc#jms-annotated-support[jms-annotated-support] for more details.
...
xref:jms:jms-receiving:jms-receiving-async-message-listener-adapter.adoc#jms-receiving-async-message-listener-adapter[jms-receiving-async-message-listener-adapter]) on an MDP is that it must implement
