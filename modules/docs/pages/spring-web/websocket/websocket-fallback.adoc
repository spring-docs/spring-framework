...
configuration of a xref:mvc:mvc-servlet.adoc#mvc-servlet[`DispatcherServlet`]. However, Spring's WebSocket
...
If you allow cross-origin requests (see xref:websocket:websocket-server:websocket-server-allowed-origins.adoc#websocket-server-allowed-origins[websocket-server-allowed-origins]), the SockJS protocol
