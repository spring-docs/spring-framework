...
as input. See the xref:jdbc:jdbc-simple-jdbc:jdbc-params.adoc#jdbc-params[next section] for details on how to define an `SqlParameter`.
...
classes (covered in xref:jdbc:jdbc-object.adoc#jdbc-object[jdbc-object]) you can use `SqlParameter` or one of its subclasses.
...
subclasses (covered in xref:jdbc:jdbc-object.adoc#jdbc-SqlQuery[jdbc-SqlQuery]).
