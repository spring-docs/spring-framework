...
The xref:orm:orm-hibernate.adoc#orm-hibernate[orm-hibernate] section provides more details and also show these features and
...
JDBC, the `JdbcTemplate` class mentioned in a xref:jdbc:jdbc-core.adoc#jdbc-JdbcTemplate[previous section]
...
xref:orm:orm-general:orm-exception-translation.adoc#orm-exception-translation[next section] for how to get the same exception
...
For details on transaction support, see the xref:transaction:transaction.adoc#transaction[transaction] chapter.
