...
definition in the xref:orm:orm-hibernate:orm-hibernate-straight.adoc#orm-hibernate-straight[next section].
...
See xref:orm:orm-jpa:orm-jpa-hibernate.adoc#orm-jpa-hibernate[Native Hibernate Setup for JPA] for details.
====
...
NOTE: Before you continue, we are strongly encourage you to read xref:transaction:transaction-declarative.adoc#transaction-declarative[transaction-declarative]
