...
aspects of the application environment: xref:beans:beans-environment:beans-definition-profiles.adoc#beans-definition-profiles[profiles]
and xref:beans:beans-environment:beans-property-source-abstraction.adoc#beans-property-source-abstraction[properties].
...
You can use `@Profile` as a xref:beans:beans-classpath-scanning:beans-meta-annotations.adoc#beans-meta-annotations[meta-annotation] for the purpose
...
entry in JNDI (see xref:beans:beans-environment:beans-property-source-abstraction.adoc#beans-property-source-abstraction[beans-property-source-abstraction]). In integration tests, active
profiles can be declared by using the `@ActiveProfiles` annotation in the `spring-test`
module (see xref:integration-testing:testcontext-framework:testcontext-ctx-management.adoc#testcontext-ctx-management-env-profiles[context configuration with environment profiles]).
