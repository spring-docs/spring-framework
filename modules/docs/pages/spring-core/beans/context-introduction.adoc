...
As discussed in the xref:beans:beans.adoc#beans[chapter introduction], the `org.springframework.beans.factory`
...
an xref:beans:context-introduction:context-functionality-events.adoc#context-functionality-events-annotation[annotation-based model] as well as the
...
of the annotation that defines a xref:expressions:expressions.adoc#expressions[`SpEL` expression] , which should match
...
xref:beans:context-introduction:context-functionality-events.adoc#context-functionality-events-async[asynchronous listeners].
...
xref:scheduling:scheduling-annotation-support:scheduling-annotation-support-async.adoc#scheduling-annotation-support-async[regular `@Async` support].
...
yourself with Spring's `Resource` abstraction, as described in xref:resources:resources.adoc#resources[resources].
