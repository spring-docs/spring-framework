...
loaded from the supplied xref:resources:resources.adoc#resources[`Resource`] location).
...
These tags are comprehensively covered in the chapter entitled xref:aop:aop.adoc#aop[Aspect Oriented Programming with Spring].
...
specified properties file (as a xref:resources:resources.adoc#resources[Spring resource location]). This element
is a convenience mechanism that sets up a xref:beans:beans-factory-extension:beans-factory-extension-factory-postprocessors.adoc#beans-factory-placeholderconfigurer[`PropertySourcesPlaceholderConfigurer`] for you. If you need more control over the specific
...
* Spring's xref:beans:beans-basics:beans-factory-metadata.adoc#beans-factory-metadata[`@Configuration`] model
* xref:beans:beans-annotation-config.adoc#beans-annotation-config[`@Autowired`/`@Inject`] and `@Value`
* JSR-250's `@Resource`, `@PostConstruct` and `@PreDestroy` (if available)
* JPA's `@PersistenceContext` and `@PersistenceUnit` (if available)
* Spring's xref:beans:context-introduction:context-functionality-events.adoc#context-functionality-events-annotation[`@EventListener`]
...
xref:transaction:transaction-declarative:transaction-declarative-annotations.adoc#transaction-declarative-annotations[`@Transactional`] annotation;
you can use the xref:transaction:transaction-declarative:tx-decl-explained.adoc#tx-decl-explained[`<tx:annotation-driven/>`]
element for that purpose. Similarly, Spring's
xref:cache:cache-annotations.adoc#cache-annotations[caching annotations] need to be explicitly
xref:cache:cache-annotations:cache-annotation-enable.adoc#cache-annotation-enable[enabled] as well.


[[xsd-schemas-context-component-scan]]
==== Using `<component-scan/>`

This element is detailed in the section on xref:beans:beans-annotation-config.adoc#beans-annotation-config[annotation-based container configuration].


[[xsd-schemas-context-ltw]]
==== Using `<load-time-weaver/>`

This element is detailed in the section on xref:aop:aop-using-aspectj:aop-aj-ltw.adoc#aop-aj-ltw[load-time weaving with AspectJ in the Spring Framework].


[[xsd-schemas-context-sc]]
==== Using `<spring-configured/>`

This element is detailed in the section on xref:aop:aop-using-aspectj:aop-atconfigurable.adoc#aop-atconfigurable[using AspectJ to dependency inject domain objects with Spring].


[[xsd-schemas-context-mbe]]
==== Using `<mbean-export/>`

This element is detailed in the section on xref:jmx:jmx-naming:jmx-context-mbeanexport.adoc#jmx-context-mbeanexport[configuring annotation-based MBean export].
...
in xref:beans:beans-dependencies:beans-factory-properties-detailed.adoc#beans-factory-properties-detailed[dependencies and configuration in detail]
(and, indeed, in that entire xref:beans:beans.adoc#beans[chapter]).
...
in the appendix entitled xref:appendix:xml-custom.adoc#xml-custom[xml-custom]).
