...
to use the aspects in it. xref:aop:aop-using-aspectj:aop-atconfigurable.adoc#aop-atconfigurable[aop-atconfigurable] and xref:aop:aop-using-aspectj:aop-ajlib-other.adoc#aop-ajlib-other[aop-ajlib-other] discuss the
content of this library and how you can use it. xref:aop:aop-using-aspectj:aop-aj-configure.adoc#aop-aj-configure[aop-aj-configure] discusses how to
dependency inject AspectJ aspects that are woven using the AspectJ compiler. Finally,
xref:aop:aop-using-aspectj:aop-aj-ltw.adoc#aop-aj-ltw[aop-aj-ltw] provides an introduction to load-time weaving for Spring applications
that use AspectJ.
...
at the field or method level (see xref:beans:beans-annotation-config.adoc#beans-annotation-config[beans-annotation-config] for further details).
...
Environment Guide]) or load-time weaving (see xref:aop:aop-using-aspectj:aop-aj-ltw.adoc#aop-aj-ltw[aop-aj-ltw]). The
...
xref:appendix:xsd-schemas:xsd-schemas-context.adoc#xsd-schemas-context[`context` namespace]
...
Further, xref:aop:aop-using-aspectj:aop-aj-ltw.adoc#aop-aj-ltw-environments[in certain environments], this support enables
...
use @AspectJ with xref:beans:beans-java.adoc#beans-java[Java configuration]. Specifically, you can use the
`@EnableLoadTimeWeaving` annotation as an alternative to `<context:load-time-weaver/>`
(see xref:aop:aop-using-aspectj:aop-aj-ltw.adoc#aop-aj-ltw-spring[below] for details).
...
You can simply enable load-time weaving, as xref:aop:aop-using-aspectj.adoc#aop-using-aspectj[described earlier].
