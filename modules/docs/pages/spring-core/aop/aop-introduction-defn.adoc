...
* Aspect: A modularization of a concern that cuts across multiple classes.
  ...
  (the xref:aop:aop-schema.adoc#aop-schema[schema-based approach]) or regular classes annotated with the
  `@Aspect` annotation (the xref:aop:aop-ataspectj.adoc#aop-ataspectj[@AspectJ style]).
