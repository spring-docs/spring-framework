...
* Specify whether to use CGLIB (described later and see also xref:aop-api:aop-pfb:aop-pfb-proxy-types.adoc#aop-pfb-proxy-types[aop-pfb-proxy-types]).
...
* `proxyTargetClass`: `true` if the target class is to be proxied, rather than the
  ...
  are created (but see also xref:aop-api:aop-pfb:aop-pfb-proxy-types.adoc#aop-pfb-proxy-types[aop-pfb-proxy-types]).
...
* `proxyInterfaces`: An array of `String` interface names. If this is not supplied, a CGLIB
  proxy for the target class is used (but see also xref:aop-api:aop-pfb:aop-pfb-proxy-types.adoc#aop-pfb-proxy-types[aop-pfb-proxy-types]).
...
to be applied. You can find an example of using this feature in xref:aop-api:aop-pfb:aop-global-advisors.adoc#aop-global-advisors[aop-global-advisors].
