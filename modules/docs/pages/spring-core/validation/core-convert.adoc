...
xref:mvc:mvc-config:mvc-config-conversion.adoc#mvc-config-conversion[Conversion and Formatting] in the Spring MVC chapter.

In certain situations, you may wish to apply formatting during conversion. See
xref:validation:format:format-CustomFormatAnnotations.adoc#format-FormatterRegistry-SPI[format-FormatterRegistry-SPI] for details on using `FormattingConversionServiceFactoryBean`.
